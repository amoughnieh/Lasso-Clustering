...
class KNN(Model):
    def __init__(self, X_train, y_train, X_val, y_val, verbose=True):
        ...
        if self.verbose:
            print("Initializing KNN model with n_neighbors=10, p=1, weights='distance'...")
        self.clf = neighbors.KNeighborsRegressor(n_neighbors=10,
                                                 weights='distance',
                                                 p=1,
                                                 n_jobs=-1)
        ...
        val_score = self.evaluate(self.normalizer.transform(X_val), y_val)

        if self.verbose:
            print("Result on validation data:", val_score)
...
class NN(Model):
    ...
    def __build_keras_model(self):
        ...
        self.model.add(Dense(1000, kernel_initializer="uniform")) #, input_dim=1182
